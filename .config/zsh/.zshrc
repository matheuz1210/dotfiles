...
prompt() {
  ...
  echo "$FX[reset]$FG[$color]╭─────$FX[reset] $FX[bold]$FG[brightblue]%n$FX[reset] at $FX[bold]$FG[brightyellow]%~$FX[reset]"
  echo "%{$FG[$color]%}╰─ %#%{$FX[reset]%} " # color vars encased in %{...%} to prevent the cursor position being wrong
}
PS1='$(prompt)'
